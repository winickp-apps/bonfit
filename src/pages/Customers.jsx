...
export default function Customers() {
  ...
  const openEdit = (c) => { setFormName(c.name); setError(''); setModal({ mode: 'edit', customer: c }) }
  ...
  const save = async () => {
    ...
    if (modal.mode === 'add') {
      ...
      ;({ error: err } = await supabase.from('customers').update({ name }).eq('id', modal.customer.id))
    }
    setSaving(false)
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-400">{filtered.length} of {customers.length} shown</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <input
              ...
              className="border border-gray-200 rounded-lg pl-8 pr-3 py-1.5 text-base w-52 focus:ring-2 focus:ring-blue-100 focus:border-blue-400"
            />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-base focus:ring-2 focus:ring-blue-200 focus:border-blue-500"
